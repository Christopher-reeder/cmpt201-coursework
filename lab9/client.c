...
It's TCP because it is using TCP function which is connect() and socket() is
using SOCK_STREAM as a argument which is for TCP.

3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? From the user's keyboard input which is
Standard Input

4. How does the client program end? How can you tell that in the code?
When there is only 1 or less input which means when only press ENTER or press
Ctrl+D
*/
