...
By using the fcntl() syscall to set the flag as O_NONBLOCK.
What sockets are made non-blocking?
Both the Server(Listening) socket and the individual Client(Communication)
sockets are set to non-blocking mode. Why are these sockets made non-blocking?
What purpose does it serve? It prevents the threads from hanging(blocking) on
accept() or read() calls if no data is currently existing. This lets the loops
to check the run flag periodically, allowing to have a clean shutdown of the
threads when the program needs to exit.
*/
